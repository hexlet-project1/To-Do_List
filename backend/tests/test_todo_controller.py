...
def test_get_todos(client, mock_conn):
    with patch('backend.py.app.todo_controller.TodoModel.get_all_todos', 
...
def test_add_todo(client, mock_conn):
    with patch('backend.py.app.todo_controller.TodoModel.add_todo', 
              return_value=True):
...
def test_update_todo(client, mock_conn):
    with patch('backend.py.app.todo_controller.TodoModel.update_todo', 
              return_value=True):
...
def test_delete_todo(client, mock_conn):
    with patch('backend.py.app.todo_controller.TodoModel.delete_todo', 
              return_value=True):
